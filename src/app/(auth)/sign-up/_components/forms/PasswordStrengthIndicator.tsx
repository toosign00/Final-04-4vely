    ...
    <div className='mt-3 rounded-lg border bg-gray-50 p-3'>
      <div className='mb-2 flex items-center justify-between'>
        ...
        <div className='flex items-center gap-1'>
          {getStrengthIcon()}
          {strength.text && <span className={`text-xs font-medium ${strength.color}`}>{strength.text}</span>}
        </div>
      </div>
      <div className='flex space-x-1'>
        {[...Array(5)].map((_, index) => (
          <div key={index} className={`h-2 flex-1 rounded-full transition-all duration-300 ${getStrengthBarColor(index)}`} />
        ))}
      </div>
