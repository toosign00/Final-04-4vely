 ...
 * @description 서버에서 북마크된 상품 데이터를 가져와 `BookmarkProductsList` 컴포넌트에 전달하여 화면에 표시
 ...
 * @description 북마크된 상품 목록을 표시하는 페이지 컴포넌트
 *              서버 사이드에서 비동기적으로 상품 데이터를 가져와 클라이언트 컴포넌트로 전달
 * @param searchParams - URL 쿼리 파라미터 (페이지 번호 포함)
 * @returns 서버에서 렌더링된 페이지 컴포넌트를 반환
 */
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  // 서버로부터 'product' 타입의 북마크 데이터를 페이지네이션으로 가져옴
  const result = await getBookmarksFromServer('product', currentPage, 5);
  ...
  // 데이터 로딩에 성공했을 경우, `BookmarkProductsList` 컴포넌트에 데이터와 페이지네이션 정보를 전달하여 렌더링
  return <BookmarkProductsList bookmarks={result.data || []} initialPage={currentPage} total={result.total || 0} hasMore={result.hasMore || false} />;
}
