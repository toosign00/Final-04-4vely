          ...
          <CarouselContent>
            {popularPosts.map((post) => (
              <CarouselItem key={post.id} className='mb-3 px-5 sm:px-7'>
                <Card className='h-full w-full place-self-center shadow-md md:max-w-none'>
                  <Link href={post.href} className='block'>
                    ...
                    <CardContent>
                      ...
                      <CardDescription description={`[ 본문 내용 ] ${post.description}`} className='text-muted text-sm md:text-base' />
                    </CardContent>
                  </Link>
                  {/* 아바타 이미지 & 닉네임 */}
                  <div className='mb-5 px-5'>
                    ...
                    <CardFooter
                      ...
                      isLiked={isLiked[post.id] !== undefined && isLiked[post.id] !== null ? isLiked[post.id] : false}
