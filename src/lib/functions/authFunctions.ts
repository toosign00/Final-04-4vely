...
export async function checkEmailAction(email: string): Promise<EmailCheckResult> {
  try {
    ...
    console.error('[이메일 확인 액션] 오류:', error);
    return {
      ok: 0,
...
export async function checkNicknameAction(nickname: string): Promise<NicknameCheckResult> {
  try {
    ...
    console.error('[닉네임 확인 액션] 오류:', error);
    return {
      ok: 0,
...
async function signUpUser(signUpData: SignUpRequestData): Promise<SignUpResult> {
  try {
    ...
    if (!res.ok) {
      console.error('[회원가입 함수] API 오류:', data);
      return {
        ok: 0,
    ...
    console.log('[회원가입 함수] 성공:', { userId: user._id, email: user.email });
    return {
      ok: 1,
      item: user,
    };
  } catch (error) {
    console.error('[회원가입 함수] 네트워크 오류:', error);
    return {
      ok: 0,
      message: '네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요.',
    };
...
async function checkEmailAvailability(email: string): Promise<EmailCheckResult> {
  console.log(`[checkEmailAvailability] API 요청: ${API_URL}/users/email?email=${email}`);
  try {
    ...
    const data = await res.json();
    console.log('[checkEmailAvailability] API 응답 상태:', res.status);
    console.log('[checkEmailAvailability] API 응답 데이터:', data);

    if (data.ok === 1) {
      console.log('[checkEmailAvailability] 이메일 사용 가능');
      return {
        ok: 1,
        item: { available: true },
      };
    } else if (data.ok === 0) {
      console.log('[checkEmailAvailability] 이메일 이미 사용 중');
      return {
        ok: 1, // API 통신은 성공했으므로 ok: 1, 비즈니스 로직 결과는 available: false
        item: { available: false },
      };
    } else {
      console.error('[이메일 확인 함수] API 응답 형식 오류:', data);
      return {
        ok: 0,
    ...
    console.error('[이메일 확인 함수] 네트워크 오류:', error);
    return {
      ok: 0,
...
async function checkNicknameAvailability(nickname: string): Promise<NicknameCheckResult> {
  console.log(`[checkNicknameAvailability] API 요청: ${API_URL}/users/name?name=${nickname}`);
  try {
    ...
    const data = await res.json();
    console.log('[checkNicknameAvailability] API 응답 상태:', res.status);
    console.log('[checkNicknameAvailability] API 응답 데이터:', data);

    if (data.ok === 1) {
      console.log('[checkNicknameAvailability] 닉네임 사용 가능');
      return {
        ok: 1,
        item: { available: true },
      };
    } else if (data.ok === 0) {
      console.log('[checkNicknameAvailability] 닉네임 이미 사용 중');
      return {
        ok: 1, // API 통신은 성공했으므로 ok: 1, 비즈니스 로직 결과는 available: false
        item: { available: false },
      };
    } else {
      console.error('[닉네임 확인 함수] API 응답 형식 오류:', data);
      return {
        ok: 0,
        message: data.message || '서버 응답이 올바르지 않습니다.',
      };
    ...
    console.error('[닉네임 확인 함수] 네트워크 오류:', error);
    return {
      ok: 0,
      message: '네트워크 오류가 발생했습니다.',
    };
